...
// ComputeMasterFromSeed returns the master public key, master secret, and chain code in hex.
func ComputeMasterFromSeed(seed []byte, pwd string) (secret [32]byte, chainCode [32]byte) {
	masterSecret := []byte(pwd)
	secret, chainCode = i64(masterSecret, seed)
...
func DerivePrivateKeyForPath(privKeyBytes [32]byte, chainCode [32]byte, path string) ([32]byte, [32]byte, error) {
	...
	if len(path) <= 0 {
		return privKeyBytes, chainCode, nil
	}
	for _, part := range parts {
		...
		if err != nil {
			return [32]byte{}, [32]byte{}, fmt.Errorf("invalid BIP 32 path: %s", err)
		}
		if idx < 0 {
			return [32]byte{}, [32]byte{}, errors.New("invalid BIP 32 path: index negative ot too large")
		}
	...
	if n != 32 || len(data) != 32 {
		return [32]byte{}, chainCode, fmt.Errorf("expected a (secp256k1) key of length 32, got length: %v", len(data))
	}
